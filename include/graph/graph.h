...
#ifndef DRAWMESH_GRAPH_H
#define DRAWMESH_GRAPH_H
...
public:
    ...
    bool find_vertex_by_property(std::string propertyType, cv::Point propertyVal, VertexIdType& vid) {
        for (auto p : vertices_) {
            Vertex v = *(p.second.vertex);
            if (v.has_property(propertyType)) {
                cv::Point p_val = get_value<cv::Point>(v[propertyType]);
                if (p_val == propertyVal) {
                    vid = v.id();
                    return true;
                }
            }
    ...
    bool find_vertex_by_property(std::string propertyType, std::string propertyVal, VertexIdType& vid) {
        for (auto p : vertices_) {
            Vertex v = *(p.second.vertex);
            if (v.has_property(propertyType)) {
                std::string p_val = get_value<std::string>(v[propertyType]);
                if (p_val.compare(propertyVal) == 0) {
                    vid = v.id();
                    return true;
                }
            }
    ...
    std::vector<VertexIdType> find_vertices_by_property(std::string propertyType, std::string propertyVal) {
